...
var grammar = (function () {
  ...
  var raw = {
    ...
    year_rule: /^\s*(?:in (even|odd|leap|non-leap) years|every (\d+) years? since (\d{4}))/,
    easter: /^(easter|orthodox) ?([-+]?\d{1,2})?(?:days?)?/,
    islamic: /0?(\d{1,2}) (_islamicMonths)/,
    jewish: /0?(\d{1,2}) (_jewishMonths)/,
    day_dir_date: /^(?:_days _direction )?(?:_counts )?_days _direction _date/,
    date_if_then: /^(substitutes )?(?:_date)?(?:_islamic)? (and )?if ((?:(?:_weekdays),?)*) then _direction _days(?: if ((?:(?:_weekdays),?)*) then _direction _days)?(?: if ((?:(?:_weekdays),?)*) then _direction _days)?(?: if ((?:(?:_weekdays),?)*) then _direction _days)?/,
...
var parseFn = {
  ...
  dateIfThen: function (o) {
    ...
    if ((cap = grammar.date_if_then.exec(o.str))) {
      ...
      res.day = toNumber(cap.shift())
      res.hour = toNumber(cap.shift())
      res.other = parseFn.islamic({
        str: parseFn._toString(cap.shift(), ' ', cap.shift())
      })
      res.observeBoth = !!cap.shift()
      res.rules = []
      while (cap[0]) {
        res.rules.push({
          if: (cap.shift()).split(','),
