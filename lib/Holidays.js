...
Holidays.prototype = {
  ...
  getHolidays: function (year, language) {
    ...
    arr = arr.filter(function(holiday, index, holidays) {
      const holidaysWithTheSameName = holidays.filter(function(h) { 
          return h.name === holiday.name 
        });
      return holidaysWithTheSameName.indexOf(holiday) === 0;
    })
  ...
  _dateByRule: function (year, rule) {
    ...
    for (var i in _date) {
      if ((date = this._disable(_date[i], _rule.disable, _rule.enable))) {
        ...
        o = _.merge({}, o, _.omit(_rule, ['fn', 'enable', 'disable']))
        if (o.substitute && !date.substitute) {
          delete o.substitute
        }
        a.push(o)
      }
