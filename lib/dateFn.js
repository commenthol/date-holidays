  ...
  return function (year) {
    ...
    for (var y = year - 1; y <= year + 1; y++) {
      ...
      for (var i = 0; i < firstDays.length; i += 2) {
        dd = m.assign({}, d, { month: firstDays[i] + 1, day: firstDays[i + 1] })
...
function _equinox (d) {
  ...
  return function (year) {
    ...
    var d1 = m.assign({}, d, { month: date.month() + 1, day: date.date() })
    d1 = _time(d1, year)
...
function _dateIfThen (d) {
  ...
  return function (year) {
    ...
    if (d.if2) {
      o = _dateIfThen(d.if2)(year)
      // ~ console.log(o.start.toISOString(), date.toISOString())
      if (o && o.start && o.start.toISOString() !== date.toISOString()) {
        return o
      }
    }
    ...
    if (map[date.getDay()]) {
      ...
      if (d.direction === 'previous') {
        offset = -1 * ((7 + weekday - d.then) % 7)
        if (!offset) {
          offset = -7
        }
      } else {
        offset = ((7 - weekday + d.then) % 7)
        if (!offset) {
          offset = 7
        }
      }
    } else if (d.substitute) {
      return false
    }
    d = _time(d, year)
    o = {
...
function _yearRule (d, year) {
  ...
  if (d.yearRule && d.yearRule.cardinality) {
    if (d.yearRule.cardinality === 'even' && (year + 1) % 2) {
      return true
